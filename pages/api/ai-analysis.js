...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    console.wr(`Invaid method: ${eq.method}`);
  ...
  if (!userAddress || !userAddress.match(/^0x[a-fA-F0-9]{40}$/)) {
    console.warn('Invalid ormissing userAddress:', userAddress);
        ...
        messages: [
          {
            role: 'system',
            content:
              'Given these social media posts about a topic, identify which ones present counte-narratives or alternative viewpoints. Return JSON with: counter_posts (array o post indices that present different perspectives), main_narrative (dominant viewpoint), counter_themes (array of alternative viewpoints found).',
          },
